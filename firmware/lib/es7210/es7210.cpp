...
void es7210_adc_init() {
    ...
    // MCLK/LRCK = 768 → mclk_div = 0 (bypass), OSR=32
    ...
    // Bias microphone
    _write(ES7210_REG_MIC12_BIAS, 0xAA); // MIC1+MIC2 bias on
    _write(ES7210_REG_MIC34_BIAS, 0xAA); // MIC3+MIC4 bias on
...
void es7210_adc_set_gain(es7210_input_mic_t mic, es7210_gain_value_t gain) {
    // ES7210 : MIC12 dans reg 0x43, MIC34 dans reg 0x44
    // Nibble haut = MIC impair, nibble bas = MIC pair
    uint8_t reg, cur;
