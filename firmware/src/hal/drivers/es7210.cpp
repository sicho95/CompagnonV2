...
void es7210_adc_init() {
    _write(ES7210_REG_RESET, 0xFF); delay(10);
    _write(ES7210_REG_RESET, 0x41);
    _write(ES7210_REG_MCLK_DIV, 0x00);
    _write(ES7210_REG_OSR,      0x20);
    _write(ES7210_REG_CLK_CTRL, 0x01);
    _write(ES7210_REG_SDP_FMT,  0x00);
    _write(ES7210_REG_SDP_CTRL, 0x01);
...
void es7210_adc_codec_enable() {
    _write(ES7210_REG_POWER, 0x00); delay(5);
...
void es7210_adc_set_gain(es7210_input_mic_t mic, es7210_gain_value_t gain) {
    ...
    if (mic == ES7210_INPUT_MIC1 || mic == ES7210_INPUT_MIC2) {
        reg = ES7210_REG_MIC12_GAIN; cur = _read(reg);
        cur = (mic == ES7210_INPUT_MIC1) ? ((cur & 0x0F) | (gain & 0xF0))
                                         : ((cur & 0xF0) | (gain >> 4));
    } else {
        reg = ES7210_REG_MIC34_GAIN; cur = _read(reg);
        cur = (mic == ES7210_INPUT_MIC3) ? ((cur & 0x0F) | (gain & 0xF0))
                                         : ((cur & 0xF0) | (gain >> 4));
    }
    _write(reg, cur);
}
