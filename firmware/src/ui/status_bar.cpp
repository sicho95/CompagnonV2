...
// Barre 480×36 LVGL 8
// - Heure locale (RTC/NTP)
// - Icône BLE (bleu si connecté)
// - Icône WiFi (vert si connecté)
// - Jauge batterie colorée + %
//   > 60% → vert, 20-60% → orange, < 20% → rouge
// ============================================================
...
// Styles
static lv_style_t _style_bar;
static lv_style_t _style_batt_green;
static lv_style_t _style_batt_orange;
static lv_style_t _style_batt_red;
...
void status_bar_init() {
    ...
    lv_obj_set_style_text_color(_lbl_ble, lv_color_hex(0x555555), 0); // gris = déconnecté
    ...
    lv_obj_set_style_text_color(_lbl_wifi, lv_color_hex(0x555555), 0);

    // Batterie — barre + label % (droite)
    ...
    lv_obj_set_style_bg_color(_bar_batt, lv_color_hex(0x333333), LV_PART_MAIN);
...
void status_bar_tick() {
    ...
    lv_obj_set_style_text_color(_lbl_ble,
        ble_conn ? lv_color_hex(0x2979FF) : lv_color_hex(0x555555), 0);

    // ── WiFi ───────────────────────────────────────────────
    bool wifi_conn = net::wifi_is_connected();
    lv_obj_set_style_text_color(_lbl_wifi,
        wifi_conn ? lv_color_hex(0x4CAF50) : lv_color_hex(0x555555), 0);
    ...
    // Couleur dynamique
    lv_color_t col;
    if      (pct > 60) col = lv_color_hex(0x4CAF50); // vert
    else if (pct > 20) col = lv_color_hex(0xFF9800); // orange
    else               col = lv_color_hex(0xF44336); // rouge
